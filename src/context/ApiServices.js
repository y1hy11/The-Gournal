...
const enhanceTruncatedContent = (content, sourceUrl, sourceName) => {
  ...
  const hasTruncation = truncationPatterns.some(pattern => pattern.test(cleanedContent));
...
const processApiArticles = (apiArticles) => {
  return apiArticles.map((article) => {
    ...
    return {
      ...article,
      content: processedContent || article.description || 'Content not available for this article.',
...
export const newsAPI = {
  ...
  getTopHeadlines: async (category = '', pageSize = 20, page = 1) => {
    try {
      ...
      throw new Error('Unable to fetch articles. Please check your internet connection and try again.');
    }
  },
  ...
  getArticlesByCategory: async (category, pageSize = 20, page = 1) => {
    try {
      ...
      throw new Error('Unable to fetch articles by category. Please check your internet connection and try again.');
    }
  }
};
